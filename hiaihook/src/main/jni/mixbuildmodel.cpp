...
Java_com_huawei_itrustee_hiaihook_utils_ModelManager_modelCompatibilityProcessFromBuffer(JNIEnv *env, jclass type,
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_huawei_itrustee_hiaihook_utils_ModelManager_modelCompatibilityProcessFromFile(JNIEnv *env, jclass type,
    ...
    if(string(currentversion).compare("000.000.000.000") == 0){
        ...
        //如果离线模型不满足兼容性，调用BuildModel函数进行离线模型的在线编译生成兼容的离线模型。
        if(check){
            result_code = CHECK_MODEL_COMPATIBILITY_SUCCESS;
        }else{
