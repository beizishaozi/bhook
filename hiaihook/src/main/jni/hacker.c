...
#define HACKER_JNI_CLASS_NAME "com/huawei/itrustee/hiaihook/view/NativeHacker"
...
static int open_proxy_auto(const char* pathname, int flags, mode_t modes)
{
    ...
    debug("open auto", pathname, flags, fd, BYTEHOOK_RETURN_ADDRESS());
...
static int open_proxy_manual(const char* pathname, int flags, mode_t modes)
{
    ...
    debug("open manual", pathname, flags, fd, BYTEHOOK_RETURN_ADDRESS());
    return fd;
}
