...
#include "include/HIAIMixModel.h"
#include <jni.h>
...
Java_com_huawei_itrustee_hiaihook_utils_ModelManager_registerListenerJNI(JNIEnv *env, jclass obj,
                                                          jobject callbacks) {
    env->GetJavaVM(&jvm);
...
Java_com_huawei_hiaidemo_utils_ModelManager_loadModelAsync(JNIEnv *env, jclass instance,
...
extern "C"
JNIEXPORT void JNICALL
Java_com_huawei_itrustee_hiaihook_utils_ModelManager_unloadModelAsync(JNIEnv *env, jclass instance) {
...
Java_com_huawei_itrustee_hiaihook_utils_ModelManager_loadModelFromFileAsync(JNIEnv *env, jclass type,
                                                             jstring modelName_,
...
extern "C"
JNIEXPORT void JNICALL
Java_com_huawei_itrustee_hiaihook_utils_ModelManager_loadModelAsyncFromBuffer(JNIEnv *env, jclass type,
...
extern "C"
JNIEXPORT void JNICALL
Java_com_huawei_itrustee_hiaihook_utils_ModelManager_runModelAsync(JNIEnv *env, jclass type,
    ...
    for(int i = 0 ;i < inputNum; i++){
        ...
        env->DeleteLocalRef(inputdata); //dataBuff[i]数组长度和传进来的buf_[i]的长度相同：154587
    }
    ...
    for(int i = 0; i < modelTensorinfo->input_cnt; ++i)
    {
        ...
        int size = HIAI_MixTensorBuffer_GetBufferSize(inputs[i]);  //size长度为dataBuff[i]的4倍
        //LOGI("zytest ASYNC JNI layer runmodel size: %d", size);
        //LOGI("zytest ASYNC JNI layer runmodel value: %f, %f, %f, %f", dataBuff[i][4], dataBuff[i][5], dataBuff[i][6], dataBuff[i][7]);
        memcpy(in_data, dataBuff[i], size);//in_data大小是dataBuff[i]的四倍，所以后三倍的内存地址的值为0
        //LOGI("zytest ASYNC JNI layer runmodel value: %x, %x, %x, %x", in_data[4], in_data[5], in_data[6], in_data[7]);
        //比较奇怪的是，对于同一张图片，应用每次重新安装启动之后in_data[4]和in_data[6]的十六进制表示形式都不同，但是两个大小相差0x6c是固定
        //LOGI("zytest ASYNC JNI layer runmodel value: %f, %f, %f, %f", in_data[4], in_data[5], in_data[6], in_data[7]);
    }
